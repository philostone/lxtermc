...
lxtermc_app_activate(GApplication *app)
{
	...
	g_print("%s - '%s' - #wins: %i - app at: %p\n", fn, lxapp->label, numwin, (void *)app);
...
lxtermc_app_cmdline(GApplication *app, GApplicationCommandLine *cmdline)
{
	...
	if (!lxapp->cmdargs->cfg) {
		...
		g_print("%s - '%s' - no config provided, try finding user's...\n", fn, lxapp->label);
		...
		*cfg = g_build_filename(usercfgdir, LXTERMC_NAME ".conf", NULL);
		if (!g_file_test(*cfg, G_FILE_TEST_EXISTS)) {
			g_free(*cfg);
			g_print("%s - '%s' - user config not found, try finding system's...\n", fn, lxapp->label);
			*cfg = g_build_filename(LXTERMC_DATA_DIR, LXTERMC_NAME".conf", NULL);
			if (!g_file_test(*cfg, G_FILE_TEST_EXISTS)) {
				g_free(*cfg);
				g_print("%s - '%s' - no config found, will be using defaults\n", fn, lxapp->label);
				*cfg = NULL;
			}
		}
